package com.ohgiraffers.handleremthod;
...
public class FirstContoller {
    ...
    *       핸들러 메소드 매개변수호 HttpServletRequest, HttpServletResponse도 사용 가능하다.
    ...
    * */
    @PostMapping("regist")
    public String registMenu(Model model, WebRequest request) { /* 서블릿 구조와 비슷? */
        ...
        String message = name + "을(를) 신규 메뉴 목록의 " + categoryCode + "번 카테고리의 " + price + "원으로 등록하셨습니다.";
     ...
     * */
    @PostMapping("modify")
    public String modifyMenuPrice(Model model, @RequestParam(required = false) String modifyName, // @RequestParam(required = false) 각종 옵션이 있어서 거의 필수임
    ...
    *
    * @ModelAttribute의 경우 커멘드 객체를 생성하여 매개변수로 전달해 준 뒤 해당 인스턴스를 model에 담는다.
    * 화면에서 출력해보면 모델에 담겨진 값을 확인할 수 있다.
    *
    * 경우에 따라 폼에서 입력한 값을 다음 화면으로 바로 전달해야 하는 경우가 발생하는데 이 때 유용하게 사용할 수 있다.
    * @ModelAttribute("모델에 담을 key")를 지정할 수 있으며, 지정하지 않으면 타입의 앞글자를 소문자로 한 네이밍 규칙을 따른다.
    * menuDTO로 담긴다.
    * */
    @PostMapping("search")
    ...
    * */
    ...
    /*
    * 4-2. @SessionAttribute를 이용하여 Session에 값 담기 - 많이 씀
    ...
    * */
    ...
    // SessionAttribute로 등록된 값은 Session의 상태를 관리하는 SessionStatus의 SetComplete() 메소드를 호출해야 사용이 만료된다.
    @GetMapping("logout2")
    ...
    public void body(){}
    /*
    * 5. @RequestBody를 이용하는 방법 - 더 많이 씀
    * 해당 어노테이션은 http 본문 자체를 읽는 부분을 모델로 변환시켜 주는 어노태이션이다.
    *
    ...
    *
    * 추가적으로 헤더에 대한 정보도 @RequestHeader 어노테이션을 이용하여 가져올 수 있다.
    * @CookieValue를 이용해서 쿠키 정보고 쉽게 불러올 수 있다.
    * */
    @PostMapping("body")
